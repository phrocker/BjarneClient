...
public:
  ...
  std::pair<string,shared_ptr<TTransport>> getTransporter(const vector<ServerConnection> *servers, const bool preferCachedConnection)
  {
	  ...
	  set<ServerConnection> serverSet = new set<ServerConnection>(servers->begin(),servers->end());
	  map<ServerConnection,vector<ServerConnection>>::iterator it = cache->begin();
	  for(; it != cache->end(); it++)
	  {
	  ...
	  try
	  {
	    return std::make_pair<string,shared_ptr<TTransport>>(conn->getHost() + ":" + conn->getPort(), createNewTransport(conn));
  ...
  {
    	shared_ptr<TTransport> serverTransport(new TSocket(conn->getHost(), conn->getPort()));
	...
	CachedTransport *cachedTransport = new CachedTransport(transporty,*conn);
